...
class Products extends MY_Controller
{
    ...
    public function index()
    {
        ...
        $joins = ["product_details pd" => ["pd.product_id = p.id", "left"], "product_categories pc" => ["p.category_id = pc.id", "left"], "product_images pi" => ["pi.product_id = p.id", "left"]];

        $select = "p.id,p.title,p.seo_url,pi.url img_url,p.isActive";
        $distinct = true;
        $groupBy = ["p.id"];
        /**
         * Pagination
    ...
    public function product_detail($seo_url)
    {
        ...
        $wheres["pi.isCover"] = 1;
        $wheres["p.lang"] = $this->viewData->lang;
        $joins = ["product_categories pc" => ["p.category_id = pc.id", "left"], "product_images pi" => ["pi.product_id = p.id", "left"], "product_details pd" => ["pd.product_id = p.id", "left"]];
        $select = "p.pattern_id,p.pattern,p.color_id,p.color,p.dimension_id,p.dimension,p.brand_id,p.brand,p.category_id,p.category,p.barcode,p.stock,pc.title category_title,pc.seo_url category_seo_url,p.id,p.title,p.seo_url,pi.url img_url,pd.description,pd.content,pd.features,p.isActive";
        $distinct = true;
        $groupBy = ["p.id"];
        ...
        if (!empty($this->viewData->product)) :
            ...
            if (!empty($this->viewData->product)) :
                ...
                $wheres["p.category_id"] = $this->viewData->product->category_id;
